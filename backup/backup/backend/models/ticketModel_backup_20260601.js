...
exports.createTicket = async (data) => {
  ...
  const created_by = await getIdByName(
    'T_USER', 'register_id',
  ...
  const team_id   = await getIdByName('T_TEAMS',       'team_id',   'team_name',   data.team_name);
  const status_id = await getIdByName('ticket_status', 'status_id', 'status_name', data.status_name);
  const client_id = await getIdByName('T_CLIENTS',     'client_id', 'client_name', data.client_name);

  let org_id = null;
  if (data.org_name) {
    org_id = await getIdByName('T_ORGANIZATION', 'org_id', 'org_name', data.org_name);
  } else if (created_by) {
    const creatorRow = await db.query(
      `SELECT org_id FROM T_USER WHERE register_id = $1`, [created_by]
    );
  ...
  if (org_id) {
    const locRow = await db.query(
      `SELECT location_id FROM T_ORGANIZATION WHERE org_id = $1`, [org_id]
    );
    location_id = locRow.rows[0]?.location_id || null;
  }
  ...
  if (!team_id)    missing.push('team_name');
  if (!status_id)  missing.push('status_name');

  if (missing.length > 0) {
    console.log("❌ Missing Fields:", missing);
    console.log("DEBUG VALUES:", { created_by, type_id, issue_id, team_id, status_id, client_id });
...
exports.getAllTickets = async (filters = {}) => {
  try {
    ...
    if (filters.org_id) {
      values.push(Number(filters.org_id));
...
exports.assignTicket = async (data) => {
  const { ticket_id, org_name, assigned_to, estimated_end_date, remark } = data;

  let org_id = null;
  if (org_name) {
    org_id = await getIdByName('T_ORGANIZATION', 'org_id', 'org_name', org_name);
  }
